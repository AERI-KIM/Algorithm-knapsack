...
void double_item(int i, double weight[], double value[])
{
    weight[i-1] = weight[i-1]*2;
    value[i-1] = value[i-1]*2;
}
...
double knapsack_one_item_double(int max_weight, double weight[], double value[], int num)
{
    int i, max_index;
...
int main()
{
    ...
    printf("%.2lf", knapsack_one_item_double(max_weight, weight, value, (index-1)/2));
    return 0;
}
